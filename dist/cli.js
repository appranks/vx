...
var skillDir = join2(import.meta.dir, "vx");
var refsDir = join2(skillDir, "references");
var VX_SKILL = readFileSync(join2(skillDir, "SKILL.md"), "utf-8");
var VX_REFERENCES = Object.fromEntries(readdirSync(refsDir).filter((f) => f.endsWith(".md")).sort().map((f) => [f, readFileSync(join2(refsDir, f), "utf-8")]));
...
async function runInit(ctx) {
  ...
  const vxDir = join3(cwd, ".claude", "skills", "vx");
  const refsDir2 = join3(vxDir, "references");
  await mkdir2(refsDir2, { recursive: true });
  const skillResult = await writeIfNotExists(join3(vxDir, "SKILL.md"), VX_SKILL, force);
  const refResults = [];
  for (const [filename, content] of Object.entries(VX_REFERENCES)) {
    const result = await writeIfNotExists(join3(refsDir2, filename), content, force);
